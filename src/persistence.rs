...
impl MemoryStore {
    ...
    /// 2つの場面で使う。
    ///
    /// - テスト: 記憶を読むようになった結果、テストが「実行環境に既にある保存
    ///   ファイル」に左右される(実際にペットを動かした後だけ失敗する)ようになっては
    ///   いけないため、テストからは必ずこれを渡す
    /// - プレビュー(`--preview-mood`): 気分を固定した個体が保存すると、本物の
    ///   ペットが覚えた生活リズムとエネルギーを上書きしてしまう
