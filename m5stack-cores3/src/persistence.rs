...
//!
//! そこで16バイトのレコードを順に並べて埋め、セクタが満杯になったときだけ
//! 消去する。消去済みの領域には消去なしで書き込める(フラッシュの書き込みは
//! 1→0 の一方向で、0xFF から任意の値にはできる)ため成立する。256区画
//! あるので消去回数は256分の1、30秒ごとでも20年以上もつ。
//!
...
impl MemoryStore {
    ...
    fn write(&mut self, memory: PetMemory, now_unix_seconds: u64) -> Result<(), StoreError> {
        if self.next_slot >= self.slots {
            ...
            // 警告を出して動作自体は続く。
            let end = self.offset + (self.slots * RECORD_LEN) as u32;
            self.flash
