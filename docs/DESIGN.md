...
src/
  ...
  render/
    dot_grid.rs      # FieldView → ピクセルバッファ / 画面座標 → セルの逆写像
...
pub enum Touch {
    Hover { at: CellPos },   // 撫でる。触れている間ずっと弱く注入する
    Click { at: CellPos },   // 突く。その瞬間だけ強く注入する
    Leave,                   // 離れる。何も注入しない
}
```
...
| 4 | 入力IF | クリックで局所注入され場が反応する。ホバーは弱い持続注入 | 完了 |
...
撫でながら突く操作を4つの乱数系列で40ラウンド繰り返し、その後かならず生きた状態へ
戻ることを確認する。

### 未確定(実装しながら決める)

- 摂動の強さを設定ファイルに出すか — いまは `interface/pointer.rs` の定数
