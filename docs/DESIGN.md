...
**まだ振る舞いには繋いでいない**。この段階ではモデルが正しく学ぶことだけを
確かめた。予測をどう振る舞いに現れさせるか(世話が来そうな時間に先回りして
活発になる、など)は、体の安全性(崖)と legibility への影響を測りながら次に
決める。重みの保存(再起動しても覚えている)と M5Stack への組み込みも、その後に行う。
