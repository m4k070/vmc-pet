...
src/
  body/            # 体 = 場。外部依存ゼロの純粋ロジック
    ...
    lenia_body.rs    # LeniaBody: BodyPort の実装。崩壊からの復帰も持つ
...
- 摂動・echo の強さを設定ファイルに出すか — いまは `interface/pointer.rs` /
  `app.rs` の定数
- 生物の切り替え手段 — assets/animals.json には4体入っているが、起動時に
  Orbium 固定になっている
- 崩壊からの復帰の見せ方 — いまは即座に置き直している。step 5 の気分状態と
  組み合わせるなら、弱る→死ぬ→生まれ直す、という間を持たせる余地がある
- 気分状態(step 5)の変数設計
