...
ペット本体の設計を変えなかった実験(崩壊の先読み、体の規則の候補、表現軸を広げる試み)の記録は `docs/experiments/` に分けてある。目次と結論の一覧は `docs/experiments/README.md`。いまの設計の根拠になった結論だけは、第2部の「実験から分かったこと」に残してある。コードのコメントは `docs/experiments/<ファイル>「節の名前」` の形で参照している。
...
ペット本体の設計を変えなかった実験の記録は `docs/experiments/` にある(目次は
`docs/experiments/README.md`)。ここには、いまの設計の根拠になった結論だけを残す。

- **安全な範囲は、場の状態ではなくパラメータで守る**: 崩壊は前触れが短く、見える量が崩れるのは
  0.3 秒ほど前からだった。場そのものを読むモデルも、見ていない負荷や生物には移らなかった。
  パラメータが分かっていれば、O2u・OG2g・2S1v では場を足しても崩壊の確率はよりよく当たらない。
  成長の強さ 0.85 以上・テンポ ×0.5〜×1.6 を生物ごとに固定した範囲で守る、いまのやり方の根拠
  (`docs/experiments/collapse-prediction.md`)
- **下限 0.85 と、がっかりしきったテンポ ×0.6 は変えない**: 徐々に弱らせる条件では、テンポ ×0.6 の
  O2u の崖は 0.80〜0.77 で、下限から 0.05 以上離れていた。全4種のペットを放置してがっかりさせても、
  16回とも崩壊しなかった。PC の場(32×32)と M5Stack の場(43×32)の両方で確かめてあり、広い
  M5Stack の場のほうが崖から遠い(docs/experiments/rule-candidates.md「放置されてがっかりした体の余裕」)
- **体の規則は、いまの Lenia のまま**(issue #1): 時間を連続に近づけても崖は消えない。Asymptotic
  Lenia では既存の生物が膨張し、SmoothLife のグライダーはペットの場に収まらない。Glaberish は既存の
  生物をそのまま動かせるので `Lenia::step_glaberish` を実験用に取り込んだが、ペットは使っていない。
  多チャンネルは計算がカーネル本数にほぼ比例し、M5Stack に余裕が無い
  (`docs/experiments/rule-candidates.md`)
- **表現軸は、色・テンポ・身じろぎのまま**: 生き残る関数で作れる「戻れる変化」は活発さの強弱だけで、
  質の違う形への変化は一方通行だった。成長関数の中心と幅を探しても、既存の生物より大きく柔軟な体は
  見つからなかった(`docs/experiments/expression-axes.md`)
...
- **パラメータが急に変わる場面での安全** — 徐々に弱らせる条件では、放置されてがっかりした
  体は崩壊までの余裕が十分あった(テンポ ×0.6 の O2u で崖は 0.80〜0.77、
  docs/experiments/rule-candidates.md「放置されてがっかりした体の余裕」)。一方、置いた直後から
  急に弱めると、O2u の崖はテンポ ×0.5 で 0.844 まで上がる
  (docs/experiments/rule-candidates.md「成長の強さの崖は時間の刻みから来ているか」)。いまのペットで急に変わるのはプレビューで
  気分を固定した瞬間くらいで、そのときは新品の個体(成長の強さ 1.0)なので崖から遠い。
  M5Stack の場(43×32)は測った —— 徐々に弱らせる条件では PC の場より崖から遠く、下限 0.77 でも
  4種とも崩壊しなかった(PC の場では O2u が 3/8 崩壊した)。広い場のほうが余裕がある
  (docs/experiments/rule-candidates.md「放置されてがっかりした体の余裕」の追記)。
  残っているのは、どちらの場でも**急な変化での安全**を測っていないこと
