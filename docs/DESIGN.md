...
[Pet = M]  crates/vmc-pet-body(PC と M5Stack で共有、no_std 対応)
   ...
   気分(Mood: 世話の予測・期待・がっかり) -> 体のテンポ、色素への刺激、身じろぎの活発さ
...
| 期待(0〜1) | `Mood`(世話の予測) | 学んだ生活リズムで、いつもの時間に高まる | 色素が溜まり珊瑚色に色づく。弱っていても元気なときと同じ強さで身じろぎする | 世話の予測を振る舞いに繋ぐ(先回り) |
...
- **自律コントローラの身じろぎ**: 体の形の歪み(歪度)を見て、場だけを揺らす(エネルギーは回復しない)。強さはエネルギーと期待のうち大きい方で決まる
...
しれない。これは数値ではなく人の目で確かめることで、まだ確かめていない。
