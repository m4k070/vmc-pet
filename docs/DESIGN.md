...
- **がっかりした O2u は、崩壊までの余裕が小さい** — O2u の成長の強さの崖は、テンポを
  遅くするほど上がり、×0.5 で 0.844 だった(「成長の強さの崖は時間の刻みから来ているか」)。
  放置されて成長の強さが下限 0.85 まで落ち、がっかりしてテンポが ×0.6 になった状態は、
  置いた直後から急に弱める条件では崖から 0.01〜0.02 しか離れていない。徐々に弱らせる既存の
  テストは通っているが、テンポ ×0.6 での崖の位置と、ゆらぎを加えた多数回の試行での崩壊の
  しやすさは測っていない
