...
**次の一手**: 現在のコントローラは `maybe_act(field)` として**場だけ**を受け取り、
エネルギーを見ていない。つまり上記の legibility は、すべて間接的な物理
(エネルギー → growth_scale → 場の力学)から生じたもので、コントローラが
状態に応じて振る舞いを変えた結果ではない。読み取りやすさを上げる自然な道は、
コントローラの観測にエネルギーを加えることであり、これはロードマップの
「観測から行動への小さな式」そのものになる。
...
1. **永続化**(プロセスをまたぐ記憶の器) ← ここ
2. **慣れ**(同じ刺激への応答が弱まる。`TouchEcho` という「減衰する空間的な記憶」が
   すでにあるので、それを体への効き目に繋ぐ)
3. 進化的探索の整備(ランダムサーチ→世代交代、全生物での安全性検証)
4. 観測から行動への小さな式 → 学習可能なモデル
...
M5Stack 版への移植(RTC とフラッシュ)はまだ行っていない。これまでと同じく、
PC で検証してから移植する順序を踏んでいる。
...
**エネルギー(気分)は弱めていない**: いまは場への効き目だけを弱め、慣れきった
場所を叩いても機嫌は直る。「慣れた刺激は世話としても数えない」方がペットらしい
かもしれないが、弱った体を叩いても回復しないのは壊れて見える恐れがあるため、
まず見た目に現れる側だけを変えて様子を見ることにした。ここは実際に触ってみた
感触で決め直す余地がある。
