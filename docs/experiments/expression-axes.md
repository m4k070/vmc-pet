...
**問いと背景**: 自律コントローラにつなぐ表現の候補として、「触れられた方へ寄る/離れる」のような向きの
表現を考えた。いまのコントローラは、形の偏りをならす向きに軽く突くだけ。突く強さは検証済みの範囲に
留めたまま、どこを突くかを選ぶだけで、体の進む向きを導けるかを確かめる(`examples/steer_trial.rs`、
約70秒)。

**方法**:

- 生物は同梱の4種。場はペットと同じ 32×32、成長の強さ 1.0、テンポ 1.0
- 出発点は、パターンを置いて 900 + 150·k ステップ育てた状態(k = 0〜11)。目標の向きは8方位
- 重心から目標の向きへ距離 4.3 か 8 セル離れた点(前側)を、一定のステップごとに突き続ける。後ろ側を
  突くのは、反対の目標の前側を突くのと同じなので、8方位の中に含まれる
- 突く強さは3段階。コントローラの強さ(量 0.024・半径 4.8・17 ステップごと)、中間(量 0.08・
  半径 4.5・17 ステップごと)、クリックの強さ(量 0.20・半径 4.5・30 ステップごと=2秒に1回)
- 比較として、何もしない場合と、いまのコントローラの場合も測った
- 測る量は、900 ステップ(60 秒)の重心の変位と目標の向きのなす角の余弦(1 なら目標どおり、0 なら
  無関係)。出発点ごとに8方位で平均すると、もともと進んでいた向きの偏りが消えるので、その最小〜
  最大も見た。あわせて崩壊(総量 5 未満)・膨張も数えた

**結果**(余弦の平均。括弧は出発点ごとの最小〜最大、壊れたのは 96 回中の回数):

| 生物 | 60 秒で動く距離 | コントローラの強さ・4.3 | 中間・4.3 | 中間・8 | クリック・4.3 | クリック・8 |
|---|---|---|---|---|---|---|
| O2u | 約 560 セル | −0.01 | −0.06(−0.19〜−0.04)・1回 | −0.14(−0.29〜−0.10)・1回 | −0.15(−0.29〜−0.06)・5回 | −0.21(−0.30〜−0.11)・6回 |
| OG2g | 約 6 セル | +0.11 | **+0.38**(+0.18〜+0.62) | +0.06(−0.34〜+0.28) | +0.24(+0.11〜+0.42) | +0.12・2回 |
| S1s | 約 320 セル | +0.10 | +0.27(+0.26〜+0.28) | +0.19 | **+0.36**(+0.34〜+0.38) | +0.26 |
| 2S1v | 約 250 セル | +0.11 | +0.30(+0.20〜+0.51) | +0.08(−0.09〜+0.20) | **+0.39**(+0.29〜+0.55) | +0.09 |

- 何もしない場合といまのコントローラは、余弦 0.00 で、動いた距離もほぼ同じだった。いまのコントローラは、
  進む向きには効いていない
- **いまのコントローラの強さでは、向きはほとんど変わらない**(余弦 0.11 以下)
- 強くすると向きが揃いはじめ、距離 4.3 のほうが効いた。S1s は出発点に依らず揃い(幅 0.04)、OG2g・
  2S1v は出発点によって 0.2〜0.6 とばらついた
- **向きの符号が生物で逆だった**。OG2g・S1s・2S1v は突いた側へ寄り、O2u は突いた側から離れた。O2u の
  向きは、どの出発点でも離れる側に揃っていた
- 既定の生物の O2u は一番導きにくかった。クリックの強さでも余弦は −0.21(平均で約 78° ずれる)で、
  96 回中 5〜6 回崩壊した。中間の強さでも 1 回崩壊した
- OG2g は60秒で約6セルしか動かない回る体で、「進み」も数セルにとどまる

**結論**:

- 突く位置で進む向きを導くことはできるが、弱い。はっきり揃うのは、いまのコントローラの 3〜8 倍の量で
  突いたときで、それでも余弦は 0.3〜0.4(平均で 65〜70° ずれる)
- その強さでは、既定の O2u が崩壊しはじめる(クリックの強さで約 5〜6%)。O2u はそもそも導きにくい
- 寄るか離れるかが生物で逆なので、「触れた方へ寄る」を作るなら、突く側を生物ごとに決めるか、突いた後の
  動きを見て側を選ぶ(閉ループ)必要がある
- **いまの強さの範囲のまま、突く位置を変えるだけで向きの表現を足すことは、既定の O2u では見込めない**。
  ペットには入れない

**限界**:

- 突く側を固定した開ループだけを試した。動きを見て側や位置を選び直す閉ループは試していない
- 余弦は 60 秒の変位の向きだけを見ていて、見ている人が「寄ってきた」と感じるかは測っていない。場は
  トーラスなので、速い体は60秒のうちに何周もする
- 強さは3段階、距離は2通り、突く間隔は2通りだけ。場は 32×32 だけ。エネルギーが減った体は試していない
