...
**追記: M5Stack の場(43×32)でも測った**

上の測定は PC の場(32×32)だけで行い、M5Stack の場は限界として残していた。粒子の体を第二の
体として載せた後も、M5Stack では Lenia の体が既定のまま動いている(docs/M5STACK.md「粒子の体を
第二の体でも動かす」)ので、同じ道具を両方の場で回し直した(`examples/disappointed_margin.rs`、
約5分)。PC の場の結果は前回と一致した(測定1の総量の底まで同じ)。

| 測定 | 条件 | PC 32×32 | M5Stack 43×32 |
|---|---|---|---|
| 1 | 全4種、いまのペット(下限 0.85、テンポ ×0.6) | 0/16。総量の底 0.95 / 0.81 / 0.74 / 0.62 | 0/16。総量の底 0.95 / **0.83** / 0.74 / 0.62 |
| 2 | O2u、下限 0.86〜0.80 × テンポ ×1.0 / ×0.6 / ×0.5 | すべて 0/16 | すべて 0/16 |
| 2 | 全4種、テンポ ×0.6、下限 0.85 / 0.83 / 0.80 | すべて 0/8 | すべて 0/8 |
| 2 | 全4種、テンポ ×0.6、下限 0.77 | O2u だけ 3/8(前回は 4/8。種が違う) | **全4種 0/8** |

- **広い場のほうが余裕がある**。PC の場では O2u が下限 0.77 で 3/8 崩壊したのに対し、M5Stack の
  場では 0/8 だった。がっかりした OG2g の総量の底も 0.81 → 0.83 と浅い
- 場が広いほうが安全なのは、トーラスの周回が長く、自分が残した尾に追いつきにくいためと考えて
  いる(「場の解像度と表示解像度」で、32×32 まで縮めても挙動は変わらないが、それより小さいと
  周辺減衰が生物を殺すと分かっているのと同じ向きの話)
- **M5Stack の体は、PC の体より崖から遠い**。下限 0.85 は両方の場で据え置いてよい

**結論**: いまの下限(成長の強さ 0.85、がっかりしきったテンポ ×0.6)は、PC・M5Stack のどちらの
場でも変えなくてよい。前節で見えた余裕の小ささ(0.01〜0.02)は、置いた直後から急に弱める厳しい
条件でのものだった。いまのペットでは、エネルギーもがっかりもゆっくりしか変わらない。

**限界**: 崩壊が0回だったことは、起きないことの証明ではない。16回で0回なら、崩壊の起きやすさは
95%の確からしさで2割未満、としか言えない。測定2は6000ステップで打ち切り、コントローラを
含めていない(測定1はコントローラ込みで、そちらは両方の場で 0/16)。崩壊の判定は総量だけ。
パラメータが急に変わる場面(プレビューで気分を固定した瞬間など)は、どちらの場でも測っていない
が、プレビューは新品の個体(エネルギー満タン、成長の強さ 1.0)で始まるので、崖からは遠い。
