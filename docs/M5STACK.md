...
- タッチは反応するようになったが、一瞬の速いタップは描画のブロッキング時間
  (実測80〜160ms)より短いと原理上検知できない。描画のブロッキング時間
  そのものを縮められないか(次項)
- `log` クレート経由の出力がなぜ本文を欠落させるかは未調査(実害は無いため
  `println!` に切り替えて回避している)
- フレームレートは差分描画で改善したが(15ステップあたり2〜4秒→1.1〜2.3秒→
  1.84秒前後で安定)、理想値(15ステップ≈0.99秒)にはまだ届いていない。
  `core-s3` の `dirty_regions` example にさらに参考にできる点がないか、SPI の
  クロック速度を上げられないかなどが次の候補。これは同時にタッチの取りこぼし
  対策にもなる
