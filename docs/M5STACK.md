...
| 4 | `embedded-graphics` で画面へドット描画 | 完了(目視確認は未) |
...
動き続け、総量も 73.3〜73.9 で健常なまま安定している。**画面への描画コードが
実際に(クラッシュせず)実行され続けていることは確認できたが、この環境には
カメラが無いため、実際に画面に正しくドットが表示されているかどうかは目視確認
できていない。** ユーザー自身の目での確認が必要。

もう一つ、実測で分かった課題がある。ステップ間の実時間を計測すると、
15ステップぶんの間隔が 2秒程度のときもあれば 4秒程度のときもあり、
一定していない(`STEP_INTERVAL=66ms` の狙いに対してかなり遅く、かつ不安定)。
毎フレーム画面全体を `clear` してから描き直す素朴な実装のコストが、その時々で
描く必要のあるセル数(Orbium 自体が脈動しながら動くため変動する)に応じて
変わっているためと考えられる。PC版では 30fps を安定して出せているのに対し、
こちらは実効フレームレートが安定しない。`core-s3` に `dirty_regions` という
example があり、変化した領域だけを再描画する仕組みが用意されている可能性が
あるため、フレームレートの安定化はそれを使う形で後続の課題とする。
...
- 常時給電か、バッテリー動作を考慮するか
- 「環境ストレス」に何を割り当てるか(前述)
- **実際に画面へ正しく描画されているかの目視確認がまだ**(このサンドボックスに
  カメラが無いため、パニックせず動き続けていることまでしか確認できていない)
- タッチ入力の確認はまだ行っていない
- `log` クレート経由の出力がなぜ本文を欠落させるかは未調査(実害は無いため
  `println!` に切り替えて回避している)
- フレームレートが不安定(15ステップあたり2〜4秒とばらつく)。毎フレーム全画面
  `clear` する素朴な実装が原因と見ている。`core-s3` の `dirty_regions` example
  を参考にした差分描画が改善策の候補
