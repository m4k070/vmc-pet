...
| 5 | 静電容量タッチを `Touch` へ変換する IF 層 | 完了(実機での指タッチ確認は未) |
...
- **実際に指で触れて体・画面が反応するかの確認がまだ**(このサンドボックスに
  画面を触る手段が無いため、タッチコントローラの初期化と、触れていない状態での
  健常な動作までしか確認できていない)
- `log` クレート経由の出力がなぜ本文を欠落させるかは未調査(実害は無いため
  `println!` に切り替えて回避している)
- フレームレートは差分描画で改善したが(15ステップあたり2〜4秒→1.1〜2.3秒)、
  理想値(15ステップ≈0.99秒)にはまだ届いていない。`core-s3` の `dirty_regions`
  example にさらに参考にできる点がないか、SPI のクロック速度を上げられないか
  などが次の候補
