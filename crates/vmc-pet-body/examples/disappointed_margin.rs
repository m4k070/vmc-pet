...
//!
//! 「成長の強さの崖は時間の刻みから来ているか」(docs/DESIGN.md)で、O2u の崖はテンポを
//! 遅くするほど上がり、×0.5 で 0.844 だった。放置されて成長の強さが下限 0.85 まで落ち、
//! がっかりしてテンポが ×0.6 になった O2u は、置いた直後から急に弱める条件では崖から
//! 0.01〜0.02 しか離れていない。実際の使われ方に近い条件で余裕を測り直す
//! (docs/DESIGN.md「放置されてがっかりした体の余裕」)。
//!
