...
#[cfg(test)]
use crate::vitality::{MAX_ENERGY, MIN_ENERGY};
...
pub struct LeniaBody {
    ...
    field: Field,
    lenia: Lenia,
    /// 場の外側に持つ少数の状態変数(docs/DESIGN.md「コンセプト」参照)。エネルギーは
    /// 場のパラメータ(成長の強さ)に効かせて「元気/放置されて弱る」を作り、テンポは体の
    /// 時間の進み方として気分を見せる。多チャンネルの体(`multichannel::MultiBody`)と同じ
    /// 決まりを使うため `vitality.rs` に切り出してある。
    vitality: Vitality,
}
...
impl LeniaBody {
    ...
    /// ただし `MIN_GROWTH_SCALE`(vitality.rs)を下限とし、実測で見つかった崩壊の崖には触れさせない。
    pub fn step(&mut self) {
        let growth_scale = self.vitality.growth_scale();
        self.lenia
            .step_at_tempo(&mut self.field, growth_scale, self.vitality.tempo());
        self.vitality.decay_step();
    }

    /// 体の時間の進み方を変える(1.0 がいつもどおり)。計測で崩壊しないと確かめた
    /// 範囲(`MIN_TEMPO`〜`MAX_TEMPO`、vitality.rs)に丸める。
    ///
    ...
    /// vitality.rs 参照)。実行中の減衰(`step`)とは別の、ゆるやかな速さで効く。
